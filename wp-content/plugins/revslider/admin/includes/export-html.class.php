...
/**
 * Export of one slider as a standalone HTML page: the rendered markup plus the scripts, styles and media
 * it needs, with all URLs rewritten to relative paths so the zip works when opened locally.
 */
class RevSliderSliderExportHtml extends RevSliderSliderExport {
	...
	/** @return void builds a standalone HTML page of the slider and offers it as a download */
	public function export_slider_html($slider_id){
		...
		//ob_get_clean() instead of ob_get_contents() + ob_clean() + ob_end_clean(): the ob_clean() was a no-op,
		//because ob_end_clean() discards the buffer anyway
		ob_start();
		$this->slider_output->set_slider_id($slider_id);
		$this->slider_output->set_markup_export(true);
		$this->slider_output->add_slider_base();
		$this->slider_html = ob_get_clean();
...
<html lang="en-US" class="no-js">
	 ...
	 * @return void rewrites asset URLs so the exported page works from the zip
	 **/
